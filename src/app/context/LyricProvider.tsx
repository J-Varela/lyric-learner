...
import type { ResultsType, StateType, ActionType } from '../types/types';
  ...
  switch (action.type) {
    case ACTION.ORIGINAL: {
      ...
      if (resArr.length && correctArr.length && wordCount) {
        percent = parseFloat(
          ((correctArr.length / resArr.length) * 100).toFixed(2)
        );
      }
...
const useLyricContext = (initState: StateType) => {
  ...
  const results = state.resultsArr.map((res, i) => {
    ...
    if (res.match)
      return (
        <span
          key={i.toString()}
          className="bg-green-500 p-2 leading-4 border rounded"
        >
          {res.word}
        </span>
      );
      ...
      return (
        <span
          key={i.toString()}
          className="bg-red-500 border p-2 leading-4 rounded"
        >
          {res.word}
        </span>
      );
  ...
  return {
    ...
    origCount: state.stats.learnerCount,
